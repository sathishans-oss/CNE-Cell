...
function handleGetChairpersonMessage(params) {
  ...
  var name = props.getProperty('CHAIRPERSON_NAME') || 'Prof. (Dr.) Meenakshi Sharma';
  var designation = props.getProperty('CHAIRPERSON_DESIG') || 'Chief Nursing Officer & Chairperson CNE Cell';
  var photoUrl = props.getProperty('CHAIRPERSON_PHOTO') || 'https://images.unsplash.com/photo-1559839734-2b71ea197ec2?auto=format&fit=crop&q=80&w=400';
...
function handleUpdateChairpersonMessage(params, session) {
  ...
  if (params.photoUrl) props.setProperty('CHAIRPERSON_PHOTO', sanitizeCellInput(params.photoUrl));
  if (params.message) props.setProperty('CHAIRPERSON_MESSAGE', sanitizeCellInput(params.message));
  
  logAuditAction('UPDATE_CHAIRPERSON_MSG', session.employeeId, 'Updated Chairperson message details', 'SUCCESS');
  return { success: true, message: 'Chairperson message updated successfully.' };
}
...
function handleGetQuickLinks(params) {
  var links = [
    ...
    {
      title: 'CNE Annual APAR Compliance Manual',
      description: 'Guidelines for submitting certified annual CNE credits for appraisal',
      url: '#'
    }
  ];
